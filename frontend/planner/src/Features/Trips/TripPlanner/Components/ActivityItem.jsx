...
export default function ActivityItem({ activity, onUpdate, onDelete }) {
    ...
    const handleKeyDown = async (e) => {
        if (e.key === "Enter") {
            if (editValue !== activity[editingField]) {
                const type = getPayloadType(editingField);
                await onUpdate({ ...activity, [editingField]: editValue }, type);
            }
    ...
    const handleDateChange = async (newDate) => {
        if (editingField && newDate && newDate !== activity[editingField]) {
            const type = getPayloadType(editingField);
            await onUpdate({ ...activity, [editingField]: newDate }, type);
        }
        setEditingField(null);
    };
